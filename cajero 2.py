...
def verificar_pin(pin_ingresado, intentos_usados):
    ...
    # Calcular intentos restantes y evitar valores negativos
    intentos_restantes = MAX_INTENTOS - intentos_usados

    # Si no quedan intentos, bloquear la tarjeta
...
def retirar(saldo, monto):
    ...
    mensaje = (
        f"✓ Retiro de {consultar_saldo(monto)} OK. "
        f"Comisión: {consultar_saldo(comision)}"
    )
    return (nuevo_saldo, mensaje, True)
...
def depositar(saldo, monto):
    ...
    mensaje = (
        f"✓ Depósito de {consultar_saldo(monto)} OK. "
        f"Saldo: {consultar_saldo(nuevo_saldo)}"
    )
    return (nuevo_saldo, mensaje, True)
...
def generar_resumen(n_dep, n_ret, total_dep, total_ret, saldo_final):
    ...
    texto = (
        "════ RESUMEN DE SESIÓN ════\n"
        f"Depósitos : {n_dep}  ({consultar_saldo(total_dep)})\n"
        f"Retiros   : {n_ret}  ({consultar_saldo(total_ret)})\n"
        "──────────────────────────\n"
        f"Saldo final: {consultar_saldo(saldo_final)}\n"
        "¡Hasta pronto!"
    )
    return texto
